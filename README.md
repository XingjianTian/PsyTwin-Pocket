...
- 心情日记：由 Sentinel 日记模板库随机抽取并支持离线回补，近 7 天在页面展示；心宠同步服务不调用 LLM 生成日记。
...
- 对已有用户补偿服务停机期间的状态变化。
- 每 4 秒对活跃心宠运行一次 tick，并广播一条带精确时间戳的真实活动日志。
- 记录活动日志、状态变化、背包发现、帮助事件。
- 日记内容由 Sentinel 模板库负责，心宠同步服务只同步已有的 `diaryDataMap`。
- 保存前会清理异常状态：三维数值限制在 0-100，日记保留最近 30 天，过期帮助事件会被过滤。
- `push` 合并时会按日记 ID、活动日志 key、帮助事件 ID 去重，避免多端或离线回传产生重复数据。
- 每 4 秒打印所有心宠状态，与 Sentinel 实时日志节拍保持一致。
...
- 当前 `config/index.js` 仍包含 AI 聊天使用的 LLM 配置项。后续应把真实密钥迁移到本地私有配置或环境变量，不要继续扩散到文档和提交说明；`server/pet-server.js` 不再包含 LLM 配置。
