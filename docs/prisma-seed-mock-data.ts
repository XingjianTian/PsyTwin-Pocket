...
// ============================================
const facultiesData = [
  { id: 'faculty_001', name: '计算机学院', campusX: 100, campusY: 200, stressIndex: 0.65, riskLevel: 'LOW' },
  { id: 'faculty_002', name: '机械学院', campusX: 150, campusY: 250, stressIndex: 0.7, riskLevel: 'MEDIUM' },
  { id: 'faculty_003', name: '文学院', campusX: 200, campusY: 150, stressIndex: 0.55, riskLevel: 'LOW' },
  { id: 'faculty_004', name: '艺术学院', campusX: 120, campusY: 180, stressIndex: 0.6, riskLevel: 'LOW' },
  { id: 'faculty_005', name: '心理健康中心', campusX: 300, campusY: 300, stressIndex: 0.4, riskLevel: 'LOW' },
];
...
const studentsData = [
  ...
  },
  {
    ...
    status: 'ACTIVE',
    badges: '[]',
    stats: JSON.stringify({ counselingCount: 0, vrSessionCount: 2, assessmentCount: 1, totalMinutes: 45 }),
    joinDate: new Date('2023-09-01'),
  },
  {
    id: 'u3',
    ...
    status: 'ACTIVE',
    badges: '[]',
    stats: JSON.stringify({ counselingCount: 1, vrSessionCount: 0, assessmentCount: 2, totalMinutes: 50 }),
    joinDate: new Date('2023-09-01'),
  },
  {
    id: 'u4',
    ...
    status: 'ACTIVE',
    badges: '[]',
    stats: JSON.stringify({ counselingCount: 0, vrSessionCount: 3, assessmentCount: 1, totalMinutes: 60 }),
    joinDate: new Date('2023-09-01'),
  },
  {
    id: 'u5',
    ...
    status: 'ACTIVE',
    badges: '[]',
    stats: JSON.stringify({ counselingCount: 1, vrSessionCount: 1, assessmentCount: 3, totalMinutes: 90 }),
    joinDate: new Date('2023-09-01'),
  },
  {
    id: 'u7',
    ...
    status: 'ACTIVE',
    badges: '[]',
    stats: JSON.stringify({ counselingCount: 0, vrSessionCount: 4, assessmentCount: 2, totalMinutes: 120 }),
    joinDate: new Date('2023-09-01'),
  },
  {
    id: 'u8',
    ...
    status: 'ACTIVE',
    badges: '[]',
    stats: JSON.stringify({ counselingCount: 2, vrSessionCount: 1, assessmentCount: 4, totalMinutes: 150 }),
    joinDate: new Date('2023-09-01'),
  },
  // 教师端学生管理中的学生
  {
...
// ============================================
const postLikesData = [
  { id: 'pl001', postId: 'post_001', studentId: 'stu001' },
  { id: 'pl002', postId: 'post_001', studentId: 'u4' },
  { id: 'pl003', postId: 'post_001', studentId: 'u5' },
  { id: 'pl004', postId: 'post_002', studentId: 'stu001' },
  { id: 'pl005', postId: 'post_002', studentId: 'u1' },
  { id: 'pl006', postId: 'post_002', studentId: 'u3' },
  { id: 'pl007', postId: 'post_002', studentId: 'u4' },
  { id: 'pl008', postId: 'post_003', studentId: 'u1' },
  { id: 'pl009', postId: 'post_003', studentId: 'u4' },
  { id: 'pl010', postId: 'post_003', studentId: 'u5' },
  { id: 'pl011', postId: 'post_003', studentId: 'u7' },
];
...
const postCollectionsData = [
  { id: 'pc001', postId: 'post_002', studentId: 'stu001' },
  { id: 'pc002', postId: 'post_002', studentId: 'u1' },
  { id: 'pc003', postId: 'post_005', studentId: 'stu001' },
  { id: 'pc004', postId: 'post_006', studentId: 'u4' },
  { id: 'pc005', postId: 'post_007', studentId: 'u5' },
];
...
async function main() {
  ...
  try {
    ...
    console.log(`✅ 插入 ${facultiesData.length} 条院系数据`);

    // 3. 插入学生数据
    console.log('👨‍🎓 插入学生数据...');
    await prisma.student.createMany({ data: studentsData });
    console.log(`✅ 插入 ${studentsData.length} 条学生数据`);

    // 4. 插入心理档案数据
    console.log('🧠 插入心理档案数据...');
    await prisma.psychProfile.createMany({ data: psychProfilesData });
    console.log(`✅ 插入 ${psychProfilesData.length} 条心理档案数据`);

    // 5. 插入教师数据
    console.log('👩‍🏫 插入教师数据...');
    await prisma.teacher.createMany({ data: teachersData });
    console.log(`✅ 插入 ${teachersData.length} 条教师数据`);
    ...
    await prisma.postLike.createMany({ data: postLikesData });
    console.log(`✅ 插入 ${postLikesData.length} 条点赞数据`);

    // 9. 插入收藏数据
    console.log('⭐ 插入收藏数据...');
    await prisma.postCollection.createMany({ data: postCollectionsData });
    console.log(`✅ 插入 ${postCollectionsData.length} 条收藏数据`);

    // 10. 插入咨询室数据
    console.log('🏥 插入咨询室数据...');
    await prisma.consultationRoom.createMany({ data: consultationRoomsData });
    console.log(`✅ 插入 ${consultationRoomsData.length} 条咨询室数据`);

    // 11. 插入预约数据
    console.log('📅 插入预约数据...');
    await prisma.appointment.createMany({ data: appointmentsData });
    console.log(`✅ 插入 ${appointmentsData.length} 条预约数据`);

    // 12. 插入聊天会话数据
    console.log('💭 插入聊天会话数据...');
    await prisma.chatSession.createMany({ data: chatSessionsData });
    console.log(`✅ 插入 ${chatSessionsData.length} 条聊天会话数据`);

    // 13. 插入聊天消息数据
    console.log('📨 插入聊天消息数据...');
    await prisma.chatMessage.createMany({ data: chatMessagesData });
    console.log(`✅ 插入 ${chatMessagesData.length} 条聊天消息数据`);

    // 14. 插入预警数据
    console.log('⚠️ 插入预警数据...');
    await prisma.warning.createMany({ data: warningsData });
    console.log(`✅ 插入 ${warningsData.length} 条预警数据`);

    // 15. 插入通知数据
    console.log('🔔 插入通知数据...');
    await prisma.studentNotification.createMany({ data: notificationsData });
    console.log(`✅ 插入 ${notificationsData.length} 条通知数据`);

    // 16. 插入排班数据
    console.log('📆 插入排班数据...');
    await prisma.schedule.createMany({ data: schedulesData });
    console.log(`✅ 插入 ${schedulesData.length} 条排班数据`);
    ...
    console.log(`   - 点赞: ${postLikesData.length}`);
    console.log(`   - 收藏: ${postCollectionsData.length}`);
