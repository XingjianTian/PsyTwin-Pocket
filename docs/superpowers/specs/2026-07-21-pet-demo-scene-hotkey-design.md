...
1. 服务器启动时默认开启演示模式；首次按 F8 会关闭。F8 是演示模式开关：开启时保存 `demo_pet` 的正常位置，并将位置固定为 `teaching_building`（教室）；关闭时恢复该快照。演示模式期间，常规 tick 仍持续模拟心情、能量、社交及其他非位置数据。
2. 本地服务器在支持终端按键输入时监听 F9。
3. F9 只操作 `demo_pet`，不影响其他用户。
4. 首次进入演示时保存 `demo_pet` 当前正常位置相关字段；如果演示已经激活，则保留首次快照，只取消尚未结束的演示计时器。
5. F9 场景切换模拟会立即将位置设为 `picnic_lawn`，递增 `stateVersion`、更新 `updatedAt`，并向 `demo_pet` 的小程序和 Unity 连接广播标准 `pet_status`。若 F8 演示模式正在运行，F9 复用首次位置快照并切入此测试序列。
6. 3 秒后把演示位置设为 `psychological_room`，再次递增版本并广播。
7. 第二阶段保持生效，不设置自动恢复计时器。
8. 重复按 F9 时，从 `picnic_lawn` 重新开始并重置 3 秒计时，但不得用演示位置覆盖首次保存的正常位置快照。
9. `pocket` 客户端断开时立即结束演示；Unity 可能同时存在 HUD 与独立场景接收器连接，因此仅在最后一个 `unity` 连接断开时结束演示。结束时恢复 F9 前保存的正常位置，递增版本，并向仍在线的 `demo_pet` 客户端广播恢复状态。
10. 因此，小程序刷新或 Unity 停止运行都会结束本次演示；重新连接后读取正常服务器状态。
...
- [x] F8 演示模式锁定 `demo_pet` 在教室，同时保持心情、能量和社交模拟；再次按下恢复正常位置，F9 仍可进入场景切换测试。*(已于 2026-07-29 通过服务端自动化测试)*
