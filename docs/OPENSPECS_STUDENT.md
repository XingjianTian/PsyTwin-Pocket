...
- [x] **发布者信息**：头像、昵称、身份标签（学生/教师）
  - 实现文件：`components/card/index.wxml`
- [x] **内容类型**：纯文字、图文混合
  - 实现文件：`components/card/index.wxml` - 图片展示
- [x] **点击卡片进入详情** ⭐ 已实现
  - 入口：`pages/home/index.wxml` - card 组件绑定 tap 事件
  - 详情页：`pages/post-detail/index`
- [x] **互动按钮**：
  - ⚠️ 点赞：列表页点击无效，详情页可点赞 (`pages/post-detail/index.js`)
  - ⚠️ 评论：详情页可查看评论和发表评论 (`pages/post-detail/index.js`)
  - ❌ 收藏：仅详情页显示按钮，未实现持久化
- [x] **发布时间**：相对时间显示
...
||------|------------|----------------|----------------|
|| 首页-心墙 | 85% | 瀑布流、下拉刷新、发布入口、详情页、点赞、评论、Mock 数据 | 上拉加载、收藏持久化 |
